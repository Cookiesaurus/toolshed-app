...
export default async function Page({ searchParams }) {
  let tool = String(searchParams.searchTerm).trim().split(" ");
  let toolSearchResults = await db.selectFromDB(`SELECT Tools.Tool_ID, Tools.Tool_Name, Home.Location_Name AS Home_Location, Current.Location_Name AS Current_Location, Tools.Brand_Name, GROUP_CONCAT(DISTINCT Types.Type_Name SEPARATOR ', ') AS Types, Tool_Statuses.Tool_Status_Details FROM Tools 
