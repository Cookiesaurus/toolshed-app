...
const InActiveMembershipReport = ({membershipData}) => {
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `Inactive-Members-Report-${reportHeader.toLocaleDateString('en-US', options)}.xlsx`, { compression: true });
  };

  const downlaodPDF = () => {
    const doc = new jsPDF({ orientation: "landscape"})
    doc.text(`Inactive-Members-Report-${reportHeader.toLocaleDateString('en-US', options)}`, 10, 10)
    ...
    //name of file
    doc.save(`Inactive-Members-Report-${reportHeader.toLocaleDateString('en-US', options)}.pdf`)
  }
...
export default InActiveMembershipReport;
